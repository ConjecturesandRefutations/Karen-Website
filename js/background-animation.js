...
function fadeInOut(t, ttl) {
  return t < ttl / 2 ? t / (ttl / 2) : 1 - (t - ttl / 2) / (ttl / 2);
}
...
function getBackgroundColor() {
  return document.documentElement.classList.contains('inverted')
    ? 'hsla(0, 0%, 100%, 1)'   // light mode background (white)
    : 'hsla(150,80%,1%,1)';    // dark mode background
}
	
function getRayHue() {
  if (document.documentElement.classList.contains('inverted')) {
    // Light theme: use darker blue/purple
    return { baseHue: 250, rangeHue: 40 };
  } else {
    // Dark theme: keep current green
    return { baseHue: 120, rangeHue: 60 };
  }
}
...
function initRay(i) {
  ...
  const { baseHue, rangeHue } = getRayHue(); // <-- use theme-based hue
  ...
  hue = baseHue + rand(rangeHue); // <-- theme-based hue
